...
# License Keys
VALID_LICENSE_KEYS = ["Pushkar931222", "NEX5-PRO-2025", "NEX5-ENTERPRISE", "DEVELOPER-ANKIT", "Prime123", "GameHindu", "pushkarbhau", "Primedragon"]
...
def create_embed(title: str, desc: str = "", color: int = COLORS['primary']) -> discord.Embed:
    ...
    embed.set_footer(text=f"⚡ {BOT_NAME} • {BOT_AUTHOR} • {datetime_.datetime.now().strftime('%Y-%m-%d %H:%M:%S')} ⚡", icon_url=THUMBNAIL_URL)
    return embed
...
def os_embed(title: str, desc: str = "") -> discord.Embed:
    return create_embed(title, desc, COLORS['os'])
...
async def show_container_mac(ctx, container: str):
    await ctx.send(embed=info_embed("Container MAC", f"MAC information for {container} is not available."))
...
class ConsoleModal(Modal):
    def __init__(self, container: str):
        ...
        self.command = TextInput(label="Command", placeholder="Enter a command", style=discord.InputTextStyle.paragraph)
        self.add_item(self.command)

    async def on_submit(self, interaction: discord.Interaction):
        await interaction.response.send_message(f"Console command sent for {self.container}", ephemeral=True)
